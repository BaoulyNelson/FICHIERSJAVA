...
public class FenetreOnglet extends Stage {

    TabPane tbPane = new TabPane();
    BorderPane bdp = new BorderPane();
    TableView<Produit> tbv = new TableView<Produit>();
    ...
    void tableView() {
        TableColumn<Produit, String> colId = new TableColumn<Produit, String>("Id");
        TableColumn<Produit, String> colNom = new TableColumn<Produit, String>("Nom");
        TableColumn<Produit, String> colDesc = new TableColumn<Produit, String>("Desciption");
        TableColumn<Produit, String> colPrix = new TableColumn<Produit, String>("Prix");
        TableColumn<Produit, String> colQte = new TableColumn<Produit, String>("Quantite");
        // ajouter les colonnes dans le TableView
        
        tbv.getColumns().add(colId);
        tbv.getColumns().add(colNom);
        tbv.getColumns().add(colPrix);
        tbv.getColumns().add(colQte);
        tbv.getColumns().add(colDesc);
    ...
    ObservableList<Produit> liste() {
        ...
        liste.add(new Produit("1000", "Mais", "Simple", 220, 180));
        liste.add(new Produit("1001", "Lait", "Alaska", 220, 150));
        liste.add(new Produit("1002", "Oeuf", "Pays", 98, 130));
        liste.add(new Produit("1003", "Viande", "Sale", 10, 100));
        liste.add(new Produit("1004", "Pain", "Rasi", 190, 100));
        liste.add(new Produit("1006", "Cassave", "Cap", 112, 180));
        return liste;
    }

    private void tableEvent() {
        Produit model = tbv.getSelectionModel().getSelectedItem();
        if (model != null) {
            btSave.setText("Update");
            txtId.setText(model.getId());
            txtNom.setText(model.getNom());
            txtPrix.setText("" + model.getPrix());
            txtQte.setText("" + model.getQuantite());
